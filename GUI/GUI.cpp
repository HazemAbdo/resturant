...
void GUI::PrintMessage(string msg, string msg1, string msg2, string msg3,
	string msg4, string msg5, string msg6,  Node<Cook*>* LastAssignedHead, string msg7, string msg8, string msg9) const	//Prints a message on status bar
{
	...
	if (msg1 != " ")
	{
		pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.2), "TS: ");
		pWind->DrawString(40, WindHeight - (int)(StatusBarHeight / 1.2), msg);


		pWind->DrawString(100, WindHeight - (int)(StatusBarHeight / 1.2), "Normal");
		pWind->DrawString(200, WindHeight - (int)(StatusBarHeight / 1.2), "Vip");
		pWind->DrawString(300, WindHeight - (int)(StatusBarHeight / 1.2), "Vegan");

		pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 1.5), "Orders");

		pWind->DrawString(110, WindHeight - (int)(StatusBarHeight / 1.5), msg1);
		pWind->DrawString(210, WindHeight - (int)(StatusBarHeight / 1.5), msg2);
		pWind->DrawString(310, WindHeight - (int)(StatusBarHeight / 1.5), msg3);

		pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 2.0), "Cooks");

		pWind->DrawString(110, WindHeight - (int)(StatusBarHeight / 2.0), msg4);
		pWind->DrawString(210, WindHeight - (int)(StatusBarHeight / 2.0), msg5);
		pWind->DrawString(310, WindHeight - (int)(StatusBarHeight / 2.0), msg6);


		pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / 3.0), "Served");
		pWind->DrawString(110, WindHeight - (int)(StatusBarHeight / 3.0), msg7);
		pWind->DrawString(210, WindHeight - (int)(StatusBarHeight / 3.0), msg8);
		pWind->DrawString(310, WindHeight - (int)(StatusBarHeight / 3.0), msg9);


		pWind->DrawString(400, WindHeight - (int)(StatusBarHeight / 1.2), "Last Assigned:");
		int iteration = 0;
		while (LastAssignedHead)
		{
			string OrderType = "";
			string CookType = "";

			int CookId = LastAssignedHead->getItem()->GetID();

			if (LastAssignedHead->getItem()->GetType() == TYPE_VIP)
				CookType = "V";
			else if (LastAssignedHead->getItem()->GetType() == TYPE_NRM)
				CookType = "N";
			else if (LastAssignedHead->getItem()->GetType() == TYPE_VGAN)
				CookType = "G";

			int OrderId = LastAssignedHead->getItem()->GetOrder()->GetID();

			if (LastAssignedHead->getItem()->GetOrder()->GetType() == TYPE_VIP)
				OrderType = "V";
			else if (LastAssignedHead->getItem()->GetOrder()->GetType() == TYPE_NRM)
				OrderType = "N";
			else if (LastAssignedHead->getItem()->GetOrder()->GetType() == TYPE_VGAN)
				OrderType = "G";


			string msg = OrderType + to_string(OrderId) + "(" + CookType + to_string(CookId) + ")";
			if (60 * iteration + 400 > 1000)
				pWind->DrawString(60 * iteration + 400, WindHeight - (int)(StatusBarHeight / 1.8), msg);
			else
				pWind->DrawString(60 * iteration + 400, WindHeight - (int)(StatusBarHeight / 1.5), msg);


			LastAssignedHead = LastAssignedHead->getNext();
			iteration++;
